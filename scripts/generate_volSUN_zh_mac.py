"""
VOL.SUN — 2026.05.03 · SUNDAY 周回顾 (ZH 简体中文) — Mac mini
주제: 这周梦之队收获了什么 — D+8~D+11 4天浓缩回顾
"""
...
def slide_zh_1():
    ...
    y += line_h
    seg3 = [("D+8 → D+11 ", BLACK), ("4天浓缩", TERRA)]
    draw_segments(draw, seg3, y, font_obj=f_head)

    f_sub = font_zh(38, bold=False)
    draw_text_centered(draw, "周日 = 回顾", 680, f_sub, TERRA)
...
def slide_zh_2():
    img = Image.new("RGB", (W, H), CREAM)
    draw = ImageDraw.Draw(img)
    draw_meta_top(draw, "WHY · 为什么周日要回顾?", "SUN", sparkle=False, latin_left=False)
    ...
    draw_text_centered(draw, "整理上周 =", 180, f_title, BLACK)
    seg = [("下周启动", BLACK), ("快3倍", TERRA)]
    draw_segments(draw, seg, 270, font_obj=f_title)

    f_subtitle = font_zh(28, bold=False)
    draw_text_centered(draw, "不是周一零点开始,而是周日21点起步", 380, f_subtitle, GRAY)

    items = [
        ("CLARITY", "零散学习汇成一线", PURPLE),
        ("MOMENTUM", "下周首要任务清晰", PINK),
        ("CADENCE", "每周节奏掌握在手", AMBER),
        ("COMPOUND", "52周 = 1年的资产", GREEN),
    ]
...
def slide_zh_3():
    img = Image.new("RGB", (W, H), CREAM)
    draw = ImageDraw.Draw(img)
    draw_meta_top(draw, "HOW · 3 STEP 回顾 · 10分钟", "SUN", sparkle=False, latin_left=False)

    f_kicker = font_latin(22, bold=True)
    draw.text((60, 120), "RETROSPECTIVE METHOD", font=f_kicker, fill=BLACK)

    f_title = font_zh(80, bold=True)
    seg = [("3", TERRA), (" STEP = ", BLACK), ("10", TERRA), (" 分钟", BLACK)]
    draw_segments(draw, seg, 180, font_obj=f_title)

    f_sub = font_zh(30, bold=False)
    draw_text_centered(draw, "学到 → 卡住 → 下周一件事", 290, f_sub, GRAY)

    steps = [
        ("STEP 1", "学到了什么 (3分钟)", "本周3个核心学习", PURPLE),
        ("STEP 2", "卡在哪里 (3分钟)", "瓶颈·未解决·失败点", PINK),
        ("STEP 3", "下周一件事 (4分钟)", "只定1个 P0 决定", GREEN),
    ]
...
def slide_zh_4():
    ...
    draw_text_centered(draw, "每日不同 VOL · 每周日回顾", 822, f_cta, WHITE)
    draw_text_centered(draw, "今日 D+11 · 4天浓缩周回顾", 868, f_cta2, WHITE)
...
if __name__ == "__main__":
    print("=" * 50)
    print("VOL.SUN — 2026.05.03 · SUNDAY 周回顾 (ZH 简体中文) Mac mini")
    print("=" * 50)
    slide_zh_1()
    slide_zh_2()
    slide_zh_3()
