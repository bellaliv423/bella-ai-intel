"""
VOL.008 D+10 vol001 톤 — 4 ZH (簡體中文) — Mac mini 版本
...
def slide_zh_1():
    ...
    draw_text_centered(draw, "週六 = SecondBrain", 680, f_sub, TERRA)
...
def slide_zh_2():
    img = Image.new("RGB", (W, H), CREAM)
    draw = ImageDraw.Draw(img)
    draw_meta_top(draw, "WHY · 為什麼必須有第二大腦", "SAT", sparkle=False, latin_left=False)

    f_kicker = font_latin(22, bold=True)
    draw.text((60, 120), "PARA + ZETTELKASTEN", font=f_kicker, fill=BLACK)

    f_title = font_zh(72, bold=True)
    draw_text_centered(draw, "知識一旦零散", 180, f_title, BLACK)
    seg = [("記憶 ", BLACK), ("0", TERRA), (" → 檢索 ", BLACK), ("∞", TERRA)]
    draw_segments(draw, seg, 270, font_obj=f_title)

    f_subtitle = font_zh(28, bold=False)
    draw_text_centered(draw, "1 筆記 + 1 連結 = 365天后秒級檢索", 380, f_subtitle, GRAY)

    items = [
        ("PROJECTS", "正在進行的工作", PURPLE),
        ("AREAS", "持續負責領域", PINK),
        ("RESOURCES", "參考資料", AMBER),
        ("ARCHIVE", "已完成存檔", GREEN),
    ]
...
def slide_zh_3():
    img = Image.new("RGB", (W, H), CREAM)
    draw = ImageDraw.Draw(img)
    draw_meta_top(draw, "HOW · 3 STEPS · 5 分鐘", "SAT", sparkle=False, latin_left=False)

    f_kicker = font_latin(22, bold=True)
    draw.text((60, 120), "HANDS-ON", font=f_kicker, fill=BLACK)

    f_title = font_zh(80, bold=True)
    seg = [("3", TERRA), (" STEPS = ", BLACK), ("5", TERRA), (" 分鐘", BLACK)]
    draw_segments(draw, seg, 180, font_obj=f_title)

    f_sub = font_zh(30, bold=False)
    draw_text_centered(draw, "捕捉 → 整理 → AI 檢索", 290, f_sub, GRAY)

    steps = [
        ("STEP 1", "捕捉 (1分鐘)", "先丟進 Daily Note", PURPLE),
        ("STEP 2", "整理 (3分鐘)", "PARA 資料夾 + 標籤 + 雙鏈", PINK),
        ("STEP 3", "AI 檢索 (1分鐘)", "用 Claude Code 即時召回", GREEN),
    ]
...
def slide_zh_4():
    ...
    seg = [("VOL.008", TERRA), (" 資料庫自動傳送", BLACK)]
    ...
    draw_text_centered(draw, "每日不同 VOL · 每日新價值", 822, f_cta, WHITE)
    draw_text_centered(draw, "今日 D+10 SecondBrain PARA 系統", 868, f_cta2, WHITE)
...
if __name__ == "__main__":
    print("=" * 50)
    print("VOL.008 D+10 — SecondBrain (ZH 簡體中文) Mac mini")
    print("=" * 50)
    slide_zh_1()
    slide_zh_2()
    slide_zh_3()
