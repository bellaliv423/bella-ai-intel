"""
VOL.008 D+5 vol001 톤 — 4 KO + 4 EN
주제: SecondBrain 옵시디언 + Claude Code 지식관리 (FRAMEWORK+STORY · 강사 아티)
"""
import sys
...
OUT_DIR = Path("D:/bella-nest/public/posts/2026-05-02")
...
def slide_ko_1():
    ...
    draw_meta_top(draw, "VOL.008 — 2026.05.02 · SATURDAY", "D+10", sparkle=True)
    ...
    y += line_h
    seg3 = [("365일 누적 시스템", BLACK)]
    draw_segments(draw, seg3, y, font_obj=f_head)

    f_sub = font(38, bold=False)
    draw_text_centered(draw, "토요일 = SecondBrain", 680, f_sub, TERRA)

    draw_sparkle_4point(draw, 110, 870, 28, fill=TERRA, w_main=8, w_diag=4)
    draw_footer(draw, 1)
    img.save(OUT_DIR / "2026-05-02_vol008_ko_01_cover.png", "PNG")
...
def slide_ko_2():
    img = Image.new("RGB", (W, H), CREAM)
    draw = ImageDraw.Draw(img)
    draw_meta_top(draw, "WHY · 왜 SecondBrain 이 필수인가", "SAT", sparkle=False)
    ...
    draw_text_centered(draw, "1 노트 + 1 링크 = 365일 후 검색 1초", 380, f_subtitle, GRAY)

    items = [
        ("PROJECTS", "현재 진행 중인 일", PURPLE),
        ("AREAS", "지속 책임 영역", PINK),
        ("RESOURCES", "참고용 자료", AMBER),
        ("ARCHIVE", "완료된 보관소", GREEN),
    ]
    ...
    draw_footer(draw, 2)
    img.save(OUT_DIR / "2026-05-02_vol008_ko_02_why.png", "PNG")
    print("OK: ko_02_why")
...
def slide_ko_3():
    img = Image.new("RGB", (W, H), CREAM)
    draw = ImageDraw.Draw(img)
    draw_meta_top(draw, "HOW · 3 STEPS · 5분", "SAT", sparkle=False)
    ...
    steps = [
        ("STEP 1", "캡처 (1분)", "Daily Note에 일단 받기", PURPLE),
        ("STEP 2", "정리 (3분)", "PARA 폴더 + 태그 + 백링크", PINK),
        ("STEP 3", "AI 검색 (1분)", "Claude Code로 즉시 호출", GREEN),
    ]
    ...
    draw_footer(draw, 3)
    img.save(OUT_DIR / "2026-05-02_vol008_ko_03_how.png", "PNG")
    print("OK: ko_03_how")
...
def slide_ko_4():
    ...
    seg = [("VOL.008", TERRA), (" 라이브러리 발송", BLACK)]
    ...
    draw_text_centered(draw, "오늘은 D+10 SecondBrain PARA 시스템", 868, f_cta2, WHITE)
    ...
    img.save(OUT_DIR / "2026-05-02_vol008_ko_04_cta.png", "PNG")
    print("OK: ko_04_cta")
...
def slide_en_1():
    ...
    seg1 = [("Your ", BLACK), ("SecondBrain", TERRA)]
    draw_segments(draw, seg1, y, font_obj=f_head)
    ...
    draw_text_centered(draw, "Saturday — SecondBrain", 680, f_sub, TERRA)

    draw_sparkle_4point(draw, 110, 870, 28, fill=TERRA, w_main=8, w_diag=4)
    draw_footer(draw, 1)
    img.save(OUT_DIR / "2026-05-02_vol008_en_01_cover.png", "PNG")
...
def slide_en_2():
    img = Image.new("RGB", (W, H), CREAM)
    draw = ImageDraw.Draw(img)
    draw_meta_top(draw, "WHY · The SecondBrain Power", "SAT", sparkle=False)
    ...
    f_subtitle = font(28, bold=False)
    draw_text_centered(draw, "1 note + 1 link = 1-sec recall after 365 days", 380, f_subtitle, GRAY)

    items = [
        ("PROJECTS", "Active work", PURPLE),
        ("AREAS", "Ongoing responsibilities", PINK),
        ("RESOURCES", "Reference material", AMBER),
        ("ARCHIVE", "Completed storage", GREEN),
    ]
    ...
    draw_footer(draw, 2)
    img.save(OUT_DIR / "2026-05-02_vol008_en_02_why.png", "PNG")
    print("OK: en_02_why")
...
def slide_en_3():
    img = Image.new("RGB", (W, H), CREAM)
    draw = ImageDraw.Draw(img)
    draw_meta_top(draw, "HOW · 3 STEPS · 5 min", "SAT", sparkle=False)
    ...
    f_sub = font(30, bold=False)
    draw_text_centered(draw, "Capture -> Organize -> AI search", 290, f_sub, GRAY)

    steps = [
        ("STEP 1", "Capture (1m)", "Drop in Daily Note first", PURPLE),
        ("STEP 2", "Organize (3m)", "PARA folders + tags + backlinks", PINK),
        ("STEP 3", "AI search (1m)", "Claude Code instant recall", GREEN),
    ]
    ...
    draw_footer(draw, 3)
    img.save(OUT_DIR / "2026-05-02_vol008_en_03_how.png", "PNG")
    print("OK: en_03_how")
...
def slide_en_4():
    ...
    seg = [("Get ", BLACK), ("VOL.008", TERRA), (" library via DM", BLACK)]
    ...
    draw_text_centered(draw, "Today: D+10 SecondBrain PARA system", 868, f_cta2, WHITE)
    draw_sparkle_4point(draw, W - 130, 865, 14, fill=WHITE, w_main=4, w_diag=2)
    ...
    img.save(OUT_DIR / "2026-05-02_vol008_en_04_cta.png", "PNG")
    print("OK: en_04_cta")


if __name__ == "__main__":
    print("=" * 50)
    print("VOL.008 D+5 Tone — SecondBrain (Obsidian + AI)")
    print("=" * 50)
    slide_ko_1()
    slide_ko_2()
    slide_ko_3()
